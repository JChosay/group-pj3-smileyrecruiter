    ...
    const renderRecentSearches = () => {
        ...
        if(recentSearches){
            list = recentSearches.map((search, i)=> {
                return (
                    <li className="list-group-item "key={i}><button className="btn btn-warning" onClick={()=>props.setSearchObj({jobTitle: search.jobTitle, location: search.location})}> {search.jobTitle} in {search.location} </button></li>
                )
            })
